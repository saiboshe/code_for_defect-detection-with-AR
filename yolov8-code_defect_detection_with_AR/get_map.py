...
if __name__ == "__main__":
    ...
    # --------------------------------------------------------------------------------------#
    #   MINOVERLAP用于指定想要获得的mAP0.x，mAP0.x的意义是什么请百度一下。
    ...
    #   因此，confidence的值应当设置得尽量小，以获得全部可能的预测框。
    #
    ...
    confidence = 0.001
    # --------------------------------------------------------------------------------------#
    #   预测时使用到的非极大抑制（NMS）值，越大表示非极大抑制越不严格。
    #
    #   该值一般不调整。
    # --------------------------------------------------------------------------------------#
    nms_iou = 0.5
    ...
    # 修改后的真实框获取部分，适用于LabelMe标注格式
    if map_mode == 0 or map_mode == 2:
        print("Get ground truth result.")
        for image_id in tqdm(image_ids):
            xml_file = os.path.join(VOCdevkit_path, "VOC2007/Annotations", image_id + ".xml")
            try:
            ...
            with open(os.path.join(map_out_path, "ground-truth", image_id + ".txt"), "w") as new_f:
                if shapes is None:
                    continue
                for item in shapes.findall('item'):
                    ...
                    if label not in class_names:
                        continue
                    shape_type_elem = item.find('shape_type')
                    if shape_type_elem is None or shape_type_elem.text.lower() != 'rectangle':
                        continue
                    points_elem = item.find('points')
                    if points_elem is None:
                        continue
                    point_items = points_elem.findall('item')
                    if len(point_items) != 2:
                        continue
                    points = []
                    for pt in point_items:
                        subitems = pt.findall('item')
                        if len(subitems) < 2:
                            continue
                        try:
                            x = float(subitems[0].text)
                            y = float(subitems[1].text)
                        except Exception:
                            continue
                        points.append([x, y])
                    if len(points) != 2:
                    ...
                    bottom = int(max(points[0][1], points[1][1]))
                    new_f.write("%s %d %d %d %d\n" % (label, left, top, right, bottom))
